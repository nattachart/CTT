...
#ifdef _DEBUG
		if(errorLW == 0){
			//sprintf(buffer, "The channel %d is set to %f MHz.", ch, lwFreqs[ch-1]/1000000.0);
			USB.printf("The channel %d is set to %f MHz.\n",ch , lwFreqs[ch-1]/1000000.0);
			//USB.printf("The channel %d is set to %f MHz.\n", ch, lwFreqs[ch-1]/1000000.0);
			//USB.print(F("The channel "));     
			//USB.print(ch);
			//USB.print(F(" is set to "));
			//USB.print(lwFreqs[3]/1000000.0);
			//USB.println(F(" MHz."));
		} else {
			//USB.printf("Error when setting the frequency channel %d. Error code: %d", ch, errorLW);
			//USB.printf("Error when setting the frequency channel . ");
			//USB.print(F("Error when setting the frequency channel ")); 
			//USB.print(ch);
			USB.print(F(". Error code: "));
			USB.println(errorLW, DEC);
...
void transmitAndReceive()
{
	...
	frame.addSensor(SENSOR_BATT_ADC, nodeMode);
#ifdef _DEBUG
	frame.showFrame();
#endif
	//Switch on LoRaWAN
